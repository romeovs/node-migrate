...
var EventEmitter = require('events').EventEmitter
  ...
  , AsyncMigration = require('./async-migration')
...
/**
 * Add a promise based migration (a migration whose functions return promises instead
 * of accepting a callback).
 *
 * @param {String} title
 * @param {Function} upAsync
 * @param {Function} downAsync
 * @api public
 */

Set.prototype.addAsyncMigration = function(title, upAsync, downAsync){
  this.migrations.push(new AsyncMigration(title, upAsync, downAsync));
};
...
Set.prototype.save = function(fn){
  ...
  this.state.save({
    pos: self.pos,
    migrations: self.migrations.map(function (m) { return { title: m.title } }),
  }, function (err) {
    self.emit('save');
    fn(err);
  });
